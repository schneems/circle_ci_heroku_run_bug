...
const debug = debug_1.default('heroku:run');
...
class Dyno extends stream_1.Duplex {
    ...
    async start() {
        ...
        if (this.opts.showStatus) {
            cli_ux_1.default.action.start(`Running ${color_1.default.cyan.bold(this.opts.command)} on ${color_1.default.app(this.opts.app)}`);
        }
        await this._doStart();
    }
    ...
    attach() {
        ...
        if (this.dyno && this.dyno.attach_url) {
            ...
            this.legacyUri = url_1.parse(this.dyno.attach_url);
        }
        if (this._useSSH) {
            this.p = this._ssh();
        }
    ...
    _rendezvous() {
        return new Promise((resolve, reject) => {
            ...
            this.reject = reject;
            if (this.opts.showStatus) {
                cli_ux_1.default.action.status = this._status('starting');
            }
            ...
            c.on('connect', () => {
                ...
                c.write(pathnameWithSearchParams.substr(1) + '\r\n', () => {
                    if (this.opts.showStatus) {
                        cli_ux_1.default.action.status = this._status('connecting');
                    }
                });
            });
    ...
    async _ssh(retries = 20) {
        ...
        try {
            ...
            this.dyno = dyno;
            cli_ux_1.default.action.stop(this._status(this.dyno.state));
    ...
    _handle(localServer) {
        ...
        this.uncork();
        if (this.opts.listen) {
            cli_ux_1.default.log(`listening on port ${host}:${port} for ssh client`);
        }
        else {
            ...
            const sshProc = child_process_1.spawn('ssh', params, { stdio });
    ...
    _env() {
        const c = this.opts.env ? helpers_1.buildEnvFromFlag(this.opts.env) : {};
    ...
    _readData(c) {
        ...
        return data => {
            ...
            if (c && firstLine) {
                if (this.opts.showStatus)
                    cli_ux_1.default.action.stop(this._status('up'));
                firstLine = false;
                this._readStdin(c);
                return;
            }
    ...
    _notify() {
        try {
            ...
            notifications_1.notify(notification);
        }
        catch (error) {
